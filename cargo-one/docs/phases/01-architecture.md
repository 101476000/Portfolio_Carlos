...
| 7 | Customs | `services/customs-service` | `CustomsDeclaration` (CAD) | DutyCalculation, ComplianceDocument | Downstream de Shipment (necesita datos de la carga/factura comercial); Anti-Corruption Layer hacia CBSA/CARM y ACE/CBP vía `integrations/carm-cbsa`, `integrations/ace-cbp`. **Checkpoint humano obligatorio** (ver `CLAUDE.md`) |
